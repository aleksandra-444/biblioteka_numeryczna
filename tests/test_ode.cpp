/**
 * @file test_ode.cpp
 * @brief Testy jednostkowe solverów równań różniczkowych zwyczajnych
 *
 * Kompilacja:  g++ -std=c++17 -I../include test_ode.cpp ../src/ode.cpp -o test_ode
 * Uruchomienie:    ./test_ode
 */
...
{
    cout << "\n[euler_solve]:\n";
    // y(1) powinno być bliskie exp(-1) ~ 0.3679
    double y = euler_solve(f_decay, 1.0, 1.0, 10000);
    check(fabs(y - exp(-1.0)) < 1e-4, "zanik y(1) bliskie exp(-1), tol 1e-4");

    // Liniowa prawa strona: y(3) = 9
...
void test_heun()
{
    ...
    check(fabs(y - exp(-1.0)) < 1e-6, "Heun: zanik y(1) bliskie exp(-1), tol 1e-6");
...
void test_midpoint()
{
    ...
    check(fabs(y - exp(-1.0)) < 1e-6, "Punkt srodkowy: zanik y(1), tol 1e-6");

    double y2 = midpoint_solve(f_linear, 0.0, 3.0, 1000);
    check(fabs(y2 - 9.0) < 1e-6, "Punkt srodkowy: y'=2t, y(3)=9, tol 1e-6");
}

void test_rk4()
{
    cout << "\n[rk4_solve]:\n";
    // RK4 powinien być bardzo dokładny nawet przy małej liczbie kroków
    double y = rk4_solve(f_decay, 1.0, 1.0, 100);
    check(fabs(y - exp(-1.0)) < 1e-10, "RK4: zanik y(1), tol 1e-10");
...
// ─────────────────────────────────────────────────────────────────────────
// Porządek dokładności: Euler < Heun ~ Punkt środkowy < RK4
// ─────────────────────────────────────────────────────────────────────────
void test_accuracy_ordering()
{
    cout << "\n[porzadek dokladnosci] RK4 > Heun > Euler (te same N):\n";
    int N = 50;
    ...
    check(fabs(yRK - exact) < fabs(yH - exact), "RK4 dokladniejszy niz Heun");
    check(fabs(yH - exact) < fabs(yE - exact), "Heun dokladniejszy niz Euler");
}

// ─────────────────────────────────────────────────────────────────────────
// ode_mse: MSE dla RK4 musi być o rzędy wielkości niższe niż dla Eulera
// ─────────────────────────────────────────────────────────────────────────
void test_ode_mse()
{
    cout << "\n[ode_mse]:\n";
    ...
    check(mseRK < mseE, "MSE RK4 < MSE Eulera");
    check(mseRK < 1e-15, "MSE RK4 < 1e-15");
}

// ─────────────────────────────────────────────────────────────────────────
// Prawo stygnięcia (Stefan-Boltzmann):
// dT/dt = -alpha * T^4,   T(0) = 5903 K
// Rozwiązanie dokładne: T(t) = T0*10000 / (1851232043943*t + 1e12)^(1/3)
// ─────────────────────────────────────────────────────────────────────────
void test_cooling_law()
{
    cout << "\n[prawo stygniecia – fizyczne RRZ]:\n";
    ...
    check(fabs(T_rk4 - T_exact) < 1.0, "stygniecie: blad RK4 < 1 K przy t=5903 s");

    double mseE = ode_mse(f_cool, exact, T0, t_end, 10000, 0);
    double mseRK = ode_mse(f_cool, exact, T0, t_end, 10000, 3);
    check(mseRK < mseE, "stygniecie: MSE RK4 < MSE Eulera");
}
